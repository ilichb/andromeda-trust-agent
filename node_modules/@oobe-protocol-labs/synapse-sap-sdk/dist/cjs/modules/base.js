...
class BaseModule {
    ...
    async requireAccountExists(accountName, address, onMissing) {
        ...
        if (acc == null) {
            (0, anchor_errors_1.throwPredicted)(onMissing.predicted, onMissing.hint ?? `${accountName} ${address.toBase58()} not found`);
        }
        return acc;
    }
    /**
    ...
    async requireAccountAbsent(accountName, address, hint) {
        ...
        if (acc !== null) {
            throw new anchor_errors_1.SapPreflightError(`${accountName} ${address.toBase58()} already exists. ${hint}`, 0, "AccountAlreadyInUse", hint);
        }
    }
    /**
    ...
    builder, label) {
        try {
            ...
            const decoded = (0, anchor_errors_1.decodeAnchorError)(err);
            if (decoded) {
                throw new anchor_errors_1.SapPreflightError(`[${label}] simulation failed: ${decoded.friendly}`, decoded.code, decoded.name, decoded.logs?.slice(0, 6).join(" | "));
            }
            throw err;
        }
    }
