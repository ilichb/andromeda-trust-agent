...
const anchor_errors_1 = require("../utils/anchor-errors");
/**
 * @name VaultModule
 ...
 * ```
 */
class VaultModule extends base_1.BaseModule {
    // ── PDA helpers ──────────────────────────────────────
    /**
     * @name deriveVault
    ...
    deriveVault(agentPda) {
        return (0, pda_1.deriveVault)(agentPda);
    }
    /**
     * @name deriveSession
    ...
    deriveSession(vaultPda, sessionHash) {
        return (0, pda_1.deriveSession)(vaultPda, sessionHash);
    }
    // ── Vault Lifecycle ──────────────────────────────────
    /**
     * @name initVault
    ...
    async openSession(sessionHash) {
        const [agentPda] = (0, pda_1.deriveAgent)(this.walletPubkey);
        const [vaultPda] = (0, pda_1.deriveVault)(agentPda);
        const [sessionPda] = (0, pda_1.deriveSession)(vaultPda, new Uint8Array(sessionHash));
        return this.methods
    ...
    async closeEpochPage(sessionPda, epochIndex) {
        const [epochPda] = (0, pda_1.deriveEpochPage)(sessionPda, epochIndex);
        return this.methods
    ...
    async addDelegate(delegatePubkey, permissions, expiresAt) {
        const [agentPda] = (0, pda_1.deriveAgent)(this.walletPubkey);
        const [vaultPda] = (0, pda_1.deriveVault)(agentPda);
        const [delegatePda] = (0, pda_1.deriveVaultDelegate)(vaultPda, delegatePubkey);
        ...
        if (exp <= nowSec) {
            (0, anchor_errors_1.throwPredicted)("DelegateExpired", `expiresAt ${exp} is in the past (now ${nowSec})`);
        }
        const maxExp = nowSec + BigInt(payments_1.MAX_DELEGATE_DURATION_SECS);
        if (exp > maxExp) {
            (0, anchor_errors_1.throwPredicted)("DelegateExpiryInvalid", `expiresAt ${exp} > now+MAX (${maxExp}); cap is ${payments_1.MAX_DELEGATE_DURATION_SECS}s (365 days)`);
        }
        if (permissions === 0) {
            (0, anchor_errors_1.throwPredicted)("InvalidDelegate", "permissions bitmask cannot be 0");
        }
    ...
    async inscribeDelegated(delegateWallet, vaultPda, sessionPda, epochPagePda, args) {
        const [delegatePda] = (0, pda_1.deriveVaultDelegate)(vaultPda, delegateWallet);
        return this.methods
    ...
    async fetchVault(agentPda) {
        const [pda] = (0, pda_1.deriveVault)(agentPda);
    ...
    async fetchSession(vaultPda, sessionHash) {
        const [pda] = (0, pda_1.deriveSession)(vaultPda, sessionHash);
    ...
    async fetchEpochPage(sessionPda, epochIndex) {
        const [pda] = (0, pda_1.deriveEpochPage)(sessionPda, epochIndex);
    ...
    async fetchDelegate(vaultPda, delegatePubkey) {
        const [pda] = (0, pda_1.deriveVaultDelegate)(vaultPda, delegatePubkey);
