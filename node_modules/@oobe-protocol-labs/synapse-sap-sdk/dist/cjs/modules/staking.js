"use strict";
/**
 * @module staking
 * @description Agent staking lifecycle — init, deposit, request unstake,
 * complete unstake, and fetch stake accounts.
 *
 * @category Modules
 * @since v0.7.0
 * @packageDocumentation
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.StakingModule = void 0;
const web3_js_1 = require("@solana/web3.js");
const base_1 = require("./base");
const pda_1 = require("../pda");
const payments_1 = require("../constants/payments");
const anchor_errors_1 = require("../utils/anchor-errors");
/**
 * @name StakingModule
 * @description Manages agent stake accounts — init, deposit, unstake, and fetch.
 *
 * @category Modules
 * @since v0.7.0
 * @extends BaseModule
 */
class StakingModule extends base_1.BaseModule {
    // ── PDA helpers ──────────────────────────────────────
    deriveStake(agentPda) {
        return (0, pda_1.deriveStake)(agentPda);
    }
    // ── Instructions ─────────────────────────────────────
    async initStake(agentWallet, initialDeposit) {
        const [agentPda] = (0, pda_1.deriveAgent)(agentWallet);
        const [stakePda] = this.deriveStake(agentPda);
        // v0.13.0 preflights
        const want = BigInt(this.bn(initialDeposit).toString());
        if (want < payments_1.MIN_AGENT_STAKE_LAMPORTS) {
            (0, anchor_errors_1.throwPredicted)("StakeBelowMinimum", `initial deposit ${want} < MIN_AGENT_STAKE_LAMPORTS ${payments_1.MIN_AGENT_STAKE_LAMPORTS}`);
        }
        await this.requireAccountAbsent("agentStake", stakePda, "Stake already initialized — use depositStake to top up");
        return this.methods
            .initStake(this.bn(initialDeposit))
            .accounts({
            wallet: this.walletPubkey,
            agent: agentPda,
            stake: stakePda,
            systemProgram: web3_js_1.SystemProgram.programId,
        })
            .rpc();
    }
    async deposit(agentWallet, amount) {
        const [agentPda] = (0, pda_1.deriveAgent)(agentWallet);
        const [stakePda] = this.deriveStake(agentPda);
        // v0.13.0 preflight — stake must already exist; amount > 0
        if (BigInt(this.bn(amount).toString()) <= 0n) {
            (0, anchor_errors_1.throwPredicted)("InsufficientStake", "Deposit amount must be > 0");
        }
        await this.requireAccountExists("agentStake", stakePda, { predicted: "NoStakeAccount", hint: "Call initStake first" });
        return this.methods
            .depositStake(this.bn(amount))
            .accounts({
            wallet: this.walletPubkey,
            agent: agentPda,
            stake: stakePda,
            systemProgram: web3_js_1.SystemProgram.programId,
        })
            .rpc();
    }
    async requestUnstake(agentWallet, amount) {
        const [agentPda] = (0, pda_1.deriveAgent)(agentWallet);
        const [stakePda] = this.deriveStake(agentPda);
        // v0.13.0 preflight — enforce MIN_STAKE floor + no double-pending
        const stake = await this.requireAccountExists("agentStake", stakePda, { predicted: "NoStakeAccount", hint: "No stake account to unstake from" });
        const want = BigInt(this.bn(amount).toString());
        if (want <= 0n)
            (0, anchor_errors_1.throwPredicted)("InsufficientStake", "Unstake amount must be > 0");
        const max = this.getMaxUnstakeLamports(stake);
        if (want > max) {
            (0, anchor_errors_1.throwPredicted)("StakeBelowMinimum", `requested ${want} would drop stake below MIN_AGENT_STAKE_LAMPORTS (max unstake = ${max})`);
        }
        if (BigInt(stake.unstakeAmount.toString()) > 0n) {
            (0, anchor_errors_1.throwPredicted)("UnstakeAlreadyPending", "A pending unstake already exists — completeUnstake first or wait for cooldown");
        }
        return this.methods
            .requestUnstake(this.bn(amount))
            .accounts({
            wallet: this.walletPubkey,
            agent: agentPda,
            stake: stakePda,
        })
            .rpc();
    }
    async completeUnstake(agentWallet) {
        const [agentPda] = (0, pda_1.deriveAgent)(agentWallet);
        const [stakePda] = this.deriveStake(agentPda);
        // v0.13.0 preflight — cooldown must have elapsed and an unstake must be pending
        const stake = await this.requireAccountExists("agentStake", stakePda, { predicted: "NoStakeAccount", hint: "No stake account" });
        const pending = BigInt(stake.unstakeAmount.toString());
        if (pending === 0n) {
            (0, anchor_errors_1.throwPredicted)("NoUnstakePending", "Call requestUnstake first");
        }
        const availableAt = BigInt(stake.unstakeAvailableAt.toString());
        const nowSec = BigInt(Math.floor(Date.now() / 1000));
        if (nowSec < availableAt) {
            (0, anchor_errors_1.throwPredicted)("UnstakeCooldownNotMet", `available at unix ${availableAt}, now ${nowSec} (Δ ${availableAt - nowSec}s)`);
        }
        return this.methods
            .completeUnstake()
            .accounts({
            wallet: this.walletPubkey,
            agent: agentPda,
            stake: stakePda,
        })
            .rpc();
    }
    // ── Fetchers ─────────────────────────────────────────
    async fetch(agentPda) {
        const [pda] = this.deriveStake(agentPda);
        return this.fetchAccount("agentStake", pda);
    }
    async fetchNullable(agentPda) {
        const [pda] = this.deriveStake(agentPda);
        return this.fetchAccountNullable("agentStake", pda);
    }
    async fetchByPda(stakePda) {
        return this.fetchAccount("agentStake", stakePda);
    }
    // ── v0.11 helpers ────────────────────────────────────────────
    /**
     * Compute the maximum amount that can be requested via `requestUnstake`
     * without breaching the on-chain {@link MIN_AGENT_STAKE_LAMPORTS} floor.
     *
     * Returns 0 if the agent is already at (or below) the floor.
     *
     * @since v0.11.0
     */
    getMaxUnstakeLamports(stake) {
        const staked = BigInt(stake.stakedAmount.toString());
        if (staked <= payments_1.MIN_AGENT_STAKE_LAMPORTS)
            return 0n;
        return staked - payments_1.MIN_AGENT_STAKE_LAMPORTS;
    }
    /**
     * Returns the lamport delta the agent must `deposit` to satisfy the
     * per-escrow stake-coverage requirement for `escrowLamports`. 0 if already
     * sufficient. Useful for a UI "top up to open this escrow" button.
     *
     * Mirrors the on-chain check in `create_escrow_v2` (v0.11 H-1).
     *
     * @since v0.11.0
     */
    getRequiredTopUp(stake, escrowLamports) {
        const required = (0, payments_1.computeRequiredStakeLamports)(escrowLamports);
        const have = BigInt(stake.stakedAmount.toString());
        return have >= required ? 0n : required - have;
    }
}
exports.StakingModule = StakingModule;
//# sourceMappingURL=staking.js.map