...
const volume_curve_1 = require("../utils/volume-curve");
/**
 * @name EscrowV2Module
...
class EscrowV2Module extends base_1.BaseModule {
    ...
    async create(agentWallet, args, splAccounts = []) {
        ...
        if (security === 0) {
            throw new Error("createEscrowV2: settlementSecurity=0 (SelfReport) is deprecated since " +
                "v0.7. Use 1 (CoSigned) or 2 (DisputeWindow). On-chain rejects with " +
                "SelfReportDeprecated.");
        }
        if (security === 1 && !args.coSigner) {
            throw new Error("createEscrowV2: settlementSecurity=1 (CoSigned) requires `coSigner` " +
        ...
        if (security === 2) {
            ...
            if (slots < 1n) {
                throw new Error("createEscrowV2: settlementSecurity=2 (DisputeWindow) requires " +
        ...
        if (security !== 0 && security !== 1 && security !== 2) {
            throw new Error(`createEscrowV2: settlementSecurity must be 1 (CoSigned) or 2 (DisputeWindow), got ${String(security)}.`);
        }
        ...
        try {
            const stakeAccount = await this.fetchAccountNullable("agentStake", stakePda);
            if (stakeAccount) {
                const escrowLamports = BigInt(this.bn(args.initialDeposit).toString());
                const required = (0, payments_1.computeRequiredStakeLamports)(escrowLamports);
                const have = BigInt(stakeAccount.stakedAmount.toString());
                if (have < required) {
                    throw new Error(`createEscrowV2: agent stake ${have} lamports < required ${required} ` +
    ...
    async deposit(agentWallet, nonce, amount, splAccounts = []) {
        ...
        if (want <= 0n)
            (0, anchor_errors_1.throwPredicted)("InsufficientEscrowBalance", "Deposit amount must be > 0");
        const isSpl = escrow.tokenMint != null;
        if (isSpl && splAccounts.length < 4) {
            (0, anchor_errors_1.throwPredicted)("SplTokenRequired", "Pass [depositorAta, escrowAta, tokenMint, tokenProgram]");
        }
        if (!isSpl && splAccounts.length > 0) {
            (0, anchor_errors_1.throwPredicted)("InvalidTokenAccount", "SOL escrow does not accept splAccounts");
        }
        return this.methods
    ...
    async settle(depositorWallet, nonce, callsToSettle, serviceHash, splAccounts = [], opts, coSigner) {
        ...
        if (isDisputeWindow && !skipAutoPending) {
            ...
            const amount = (0, volume_curve_1.calculateSettleAmount)(escrowAcc.pricePerCall, escrowAcc.volumeCurve, totalCallsBefore, this.bn(callsToSettle));
     ...
     * @since v0.12.8
     */
    async nextSettlementIndex(agentWallet, depositorWallet, nonce) {
        const [agentPda] = (0, pda_1.deriveAgent)(agentWallet);
        const [escrowPda] = this.deriveEscrow(agentPda, depositorWallet, nonce);
        const escrow = await this.fetchAccountNullable("escrowAccountV2", escrowPda);
        if (!escrow) {
            throw new Error(`nextSettlementIndex: escrow PDA ${escrowPda.toBase58()} not found on-chain`);
        }
    ...
    async createPendingSettlement(agentWallet, depositorWallet, nonce, settlementIndex, callsToSettle, amount, serviceHash, receiptMerkleRoot = new Array(32).fill(0)) {
        ...
        if (existing) {
            ...
            throw new Error(`createPendingSettlement: pending PDA ${pendingPda.toBase58()} ` +
    ...
    async withdraw(agentWallet, nonce, amount) {
        ...
        const want = BigInt(this.bn(amount).toString());
        if (want <= 0n)
            (0, anchor_errors_1.throwPredicted)("InsufficientEscrowBalance", "Withdraw amount must be > 0");
        ...
        if (want > free) {
            (0, anchor_errors_1.throwPredicted)("InsufficientEscrowBalance", `requested ${want}, withdrawable ${free} (balance ${balance} − pending ${pending})`);
        }
        return this.methods
    ...
    async close(agentWallet, nonce = 0) {
        ...
        const pending = BigInt(escrow.pendingAmount.toString());
        if (balance !== 0n) {
            (0, anchor_errors_1.throwPredicted)("EscrowNotEmpty", `balance ${balance} > 0 — withdraw first`);
        }
        if (pending !== 0n) {
            (0, anchor_errors_1.throwPredicted)("EscrowNotClosed", `pending_amount ${pending} > 0 — finalize all PendingSettlements first or quarantine orphans via diagnoseOrphanPending`);
        }
        return this.methods
            .closeEscrowV2()
            .accounts({
