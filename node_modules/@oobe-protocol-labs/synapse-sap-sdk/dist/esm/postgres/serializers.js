...
const bn = (v) => v ? v.toString() : null;
/** Convert PublicKey to base58 string. */
const pk = (v) => v ? v.toBase58() : null;
...
/** PublicKey[] → TEXT[] */
const pks = (arr) => arr.map((p) => p.toBase58());
// ═══════════════════════════════════════════════════════════════════
//  Serializers — one per account type
// ═══════════════════════════════════════════════════════════════════
export function serializeGlobalRegistry(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
...
export function serializeAgent(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        version: d.version,
        ...
        pricing: JSON.stringify(d.pricing.map((t) => ({
            ...t,
            ...
            tokenType: enumVariant(t.tokenType),
            settlementMode: enumVariant(t.settlementMode),
        ...
        active_plugins: JSON.stringify(d.activePlugins.map((p) => ({
            pluginType: enumVariant(p.pluginType),
            pda: pk(p.pda),
        }))),
        slot,
        synced_at: new Date(),
...
export function serializeAgentStats(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        agent: pk(d.agent),
...
export function serializeFeedback(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        agent: pk(d.agent),
...
export function serializeCapabilityIndex(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
...
export function serializeProtocolIndex(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
...
export function serializeVault(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        agent: pk(d.agent),
...
export function serializeSession(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        vault: pk(d.vault),
...
export function serializeEpochPage(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
...
export function serializeDelegate(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        vault: pk(d.vault),
...
export function serializeTool(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        agent: pk(d.agent),
        ...
        http_method: enumVariant(d.httpMethod),
        category: enumVariant(d.category),
...
export function serializeCheckpoint(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        session: pk(d.session),
...
export function serializeEscrow(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        agent: pk(d.agent),
        ...
        expires_at: bn(d.expiresAt),
        volume_curve: JSON.stringify(d.volumeCurve.map((v) => ({
            afterCalls: v.afterCalls,
            pricePerCall: v.pricePerCall?.toString(),
        }))),
...
export function serializeToolCategoryIndex(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        category: enumVariant({ [d.category]: {} }) ?? String(d.category),
        tools: pks(d.tools),
        total_pages: d.totalPages,
        last_updated: bn(d.lastUpdated),
        slot,
...
export function serializeAttestation(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        agent: pk(d.agent),
...
export function serializeLedger(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        session: pk(d.session),
...
export function serializeLedgerPage(pda, d, slot) {
    return {
        pda,
        bump: d.bump,
        ledger: pk(d.ledger),
